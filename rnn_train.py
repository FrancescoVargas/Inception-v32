...
def main(input_data_dump, num_frames_per_video, batch_size, labels, model_file):
    ...
    validation_loss, number_of_epoch = model.fit(X_train, y_train, validation_set=(X_test, y_test),
              show_metric=True, batch_size=batch_size, snapshot_step=100,
              n_epoch=10)
    
    epoch_array = list(range(1, number_of_epoch+1))
    
    plt.plot(epoch_array, validation_loss, 'b', label='Validation Loss')
    plt.title('Validation Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.show()
    plt.savefig('grafico_loss8.png') 
...
if __name__ == '__main__':
    ...
    parser.add_argument("input_file_dump", help="file containing intermediate representation of gestures from inception model",default='predicted-frames-final_result-train.pkl')
    parser.add_argument("model_file", help="Name of the model file to be dumped. Model file is created inside a checkpoints folder",default='non_pool.model')
